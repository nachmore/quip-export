...
describe('methods tests', () => {
    ...
    describe('_getMatchGroups', () => {
        ...
        test('get matches for files', async () => {
            ...
            `;
            const result = quipProcessor._getMatchGroups(text,
                'href="(.*/blob/(.+)/(.+)\\?name=(.+))"',
    ...
    describe('_exportFolders', () => {
        ...
        test('reseting internal vars', async () => {
            ...
            await quipProcessor._exportFolders(['f1', 'f2']);
            expect(quipProcessor.threadsTotal).toBe(0);
            expect(quipProcessor.foldersTotal).toBe(2);
        });
    ...
    describe('_processThread', () => {
        ...
        beforeEach(() => {
            ...
            quipProcessor._processDocumentThreadDocx = jest.fn();
            quipProcessor._processDocumentThreadXlsx = jest.fn();
            thread = Object.assign({}, threads['THREAD-1']);
            quipProcessor.threadsProcessed = 0;
            quipProcessor.options.docx = false;
        });
        ...
        test("call _processDocumentThread() for docuement-type 'document' with --docx option", async () => {
            thread.thread.type = 'document';
            quipProcessor.options.docx = true;
            await quipProcessor._processThread(thread, "/aaa/");
            expect(quipProcessor._processDocumentThreadDocx).toBeCalledWith(thread, "/aaa/");
        });

        test("call _processDocumentThread() for docuement-type 'spreadsheet' with --docx option", async () => {
            thread.thread.type = 'spreadsheet';
            quipProcessor.options.docx = true;
            await quipProcessor._processThread(thread, "/aaa/");
            expect(quipProcessor._processDocumentThreadXlsx).toBeCalledWith(thread, "/aaa/");
        });
        ...
        test("call both HTML and DOCX export when both --html and --docx options are set", async () => {
            thread.thread.type = 'document';
            quipProcessor.options.html = true;
            quipProcessor.options.docx = true;
            await quipProcessor._processThread(thread, "/aaa/");
            expect(quipProcessor._processDocumentThread).toBeCalledWith(thread, "/aaa/");
            expect(quipProcessor._processDocumentThreadDocx).toBeCalledWith(thread, "/aaa/");
        ...
        test("call both HTML and XLSX export when both --html and --docx options are set for spreadsheet", async () => {
            thread.thread.type = 'spreadsheet';
            quipProcessor.options.html = true;
            quipProcessor.options.docx = true;
            await quipProcessor._processThread(thread, "/aaa/");
        ...
        test("call only HTML export when only --html option is set", async () => {
            thread.thread.type = 'document';
            quipProcessor.options.html = true;
            quipProcessor.options.docx = false;
            await quipProcessor._processThread(thread, "/aaa/");
            expect(quipProcessor._processDocumentThread).toBeCalledWith(thread, "/aaa/");
            expect(quipProcessor._processDocumentThreadDocx).not.toBeCalled();
        });

        test("call HTML export by default when no format options are set", async () => {
            thread.thread.type = 'document';
            quipProcessor.options.html = false;
            quipProcessor.options.docx = false;
            await quipProcessor._processThread(thread, "/aaa/");
            expect(quipProcessor._processDocumentThread).toBeCalledWith(thread, "/aaa/");
            expect(quipProcessor._processDocumentThreadDocx).not.toBeCalled();
    ...
    describe('_getCleanTitleForFilename', () => {
        beforeEach(() => {
            initQuipProcessor();
        });

        test('returns original title when no titlePrefix is set', () => {
            quipProcessor.options.titlePrefix = null;
            const result = quipProcessor._getCleanTitleForFilename('My Document Title');
            expect(result).toBe('My Document Title');
        });

        test('returns original title when title is null or undefined', () => {
            quipProcessor.options.titlePrefix = '[EXPORTED]';
            expect(quipProcessor._getCleanTitleForFilename(null)).toBe(null);
            expect(quipProcessor._getCleanTitleForFilename(undefined)).toBe(undefined);
        });

        test('strips prefix with space from beginning of title', () => {
            quipProcessor.options.titlePrefix = '[EXPORTED]';
            const result = quipProcessor._getCleanTitleForFilename('[EXPORTED] My Document Title');
            expect(result).toBe('My Document Title');
        });

        test('strips prefix without space from beginning of title', () => {
            quipProcessor.options.titlePrefix = '[EXPORTED]';
            const result = quipProcessor._getCleanTitleForFilename('[EXPORTED]My Document Title');
            expect(result).toBe('My Document Title');
        });

        test('returns original title when prefix is not at the beginning', () => {
            quipProcessor.options.titlePrefix = '[EXPORTED]';
            const result = quipProcessor._getCleanTitleForFilename('My [EXPORTED] Document Title');
            expect(result).toBe('My [EXPORTED] Document Title');
        });
        ...
        test('returns empty string when title is only the prefix', () => {
            quipProcessor.options.titlePrefix = '[EXPORTED]';
            const result = quipProcessor._getCleanTitleForFilename('[EXPORTED]');
            expect(result).toBe('');
        });

        test('returns empty string when title is only the prefix with space', () => {
            quipProcessor.options.titlePrefix = '[EXPORTED]';
            const result = quipProcessor._getCleanTitleForFilename('[EXPORTED] ');
            expect(result).toBe('');
        });
    ...
    describe('_getThreadMessagesHtml', () => {
        beforeEach(() => {
            ...
            quipProcessor._processReference.mockImplementation((html, reference) => {
                ...
                if(reference.referenceId === 'XXXXXXXXXX') {
                    return Promise.resolve(html + 'PROCESSED-UNKNOWN-REFERENCE');
                }
                return Promise.resolve();
            });
        ...
        test('thread with some error', async () => {
            quipProcessor.quipService.getThreadMessages.mockResolvedValue();
            let result = await quipProcessor._getThreadMessagesHtml({thread: {id: 'thread123'}}, 'Test/Programming/');
            expect(result).toBe('');
        });
    ...
    describe('_processDocumentThreadDocx', () => {
        const thread = {
            thread: {title: 'someDoc'}
        };

        beforeEach(() => {
            initQuipProcessor();
            quipProcessor.quipService.getDocx.mockResolvedValue('BLOB-DATA');
        });

        test('process document', async () => {
            await quipProcessor._processDocumentThreadDocx(thread, '/aaa/');
            expect(quipProcessor.saveCallback).toHaveBeenCalledWith('BLOB-DATA', 'someDoc.docx_SANITIZED', 'BLOB', '/aaa/');
        });
    });

    describe('_processDocumentThreadXlsx', () => {
        const thread = {
            thread: {title: 'someDoc'}
        };
        ...
        beforeEach(() => {
            initQuipProcessor();
            quipProcessor.quipService.getXlsx.mockResolvedValue('BLOB-DATA');
        });

        test('process document', async () => {
            await quipProcessor._processDocumentThreadXlsx(thread, '/aaa/');
            expect(quipProcessor.saveCallback).toHaveBeenCalledWith('BLOB-DATA', 'someDoc.xlsx_SANITIZED', 'BLOB', '/aaa/');
        });
    });
