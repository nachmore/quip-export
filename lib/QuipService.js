            ...
            if (!res.ok) {
                if (res.status === 503 || res.status === 429 || res.status == 504) {
                    ...
                    if (this._checkIfShouldRetryQuery(url)) {
                        ...
                        this.logger.debug(`HTTP ${res.status}: for ${url}, rate limit remaining: ${rateLimitRemaining} (rate limit reset: ${rateLimitReset}). Sleeping ${waitingInMs}ms / ${waitingInMin}min`);
                        return new Promise(resolve => setTimeout(() => {
