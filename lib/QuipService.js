...
const TIMES_LIMIT_503 = 100;
const SLEEP_WHEN_CHECK_USER_IS_THROTTLED = 60000;
...
class QuipService {
    ...
    async checkUser() {
        ...
        if (res.status === 429) {
            ...
            }, SLEEP_WHEN_CHECK_USER_IS_THROTTLED));
        ...
        try {
            ...
            if (!res.ok) {
                if (res.status === 503 || res.status === 429 || res.status == 504) {
                    ...
                    if (this._checkIfShouldRetryQuery(url)) {

                        const waitingInMin = waitingInMs/60000;

                        this.logger.debug(`HTTP ${res.status}: for ${url}, rate limit remaining: ${rateLimitRemaining}. Sleeping ${waitingInMs}ms / ${waitingInMin}min`);
                        return new Promise(resolve => setTimeout(() => {
    ...
    _checkIfShouldRetryQuery(url) {
        let count = this.queryRetryCounter.get(url);
        if (!count) {
            count = 0;
        }

        this.queryRetryCounter.set(url, ++count);
