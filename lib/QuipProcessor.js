...
class QuipProcessor {
    constructor(quipToken, saveCallback = () => { }, progressCallback = () => { }, phaseCallback = () => { }, options = {}, baseURL) {
        ...
        this.quipService = new QuipService(quipToken, baseURL);
    ...
    async _processReference(html, reference, pathDeepness) {
        ...
        if (referencedObject.folder) {
            ...
            //thread
            const cleanTitle = this._getCleanTitleForFilename(referencedObject.title);
            path = '../'.repeat(pathDeepness) + referencedObject.path + sanitizeFilename(cleanTitle.trim()) + '.html';
        }
    ...
    async _processDocumentThreadDocx(quipThread, path) {
        ...
        if (docx) {
            const cleanTitle = this._getCleanTitleForFilename(quipThread.thread.title);
            this.saveCallback(docx, sanitizeFilename(`${cleanTitle.trim()}.docx`), 'BLOB', path);
        }
    }
    ...
    async _processDocumentThreadXlsx(quipThread, path) {
        ...
        if (xlsx) {
            const cleanTitle = this._getCleanTitleForFilename(quipThread.thread.title);
            this.saveCallback(xlsx, sanitizeFilename(`${cleanTitle.trim()}.xlsx`), 'BLOB', path);
        }
    }
    ...
    async _processDocumentThread(quipThread, path) {
        ...
        const cleanTitle = this._getCleanTitleForFilename(quipThread.thread.title);
        this.saveCallback(wrappedHtml, sanitizeFilename(`${cleanTitle.trim()}.html`), 'THREAD', path);
    }
    ...
    async _processThread(quipThread, path) {
        ...
        const shouldExportHtml = this.options.html || (!this.options.docx && !this.options.html);
        const shouldExportDocx = this.options.docx;
        ...
        if (this.options.titlePrefix) {
            this._updateTitlePrefix(quipThread);
        }

        if (this.options.lock) {
            try {
                await this.quipService.lockThread(quipThread.thread.id);
                this.logger.success(`locked ${quipThread.thread.title}`);
            } catch (error) {
                this.logger.error(`failed to lock ${quipThread.thread.title}: ${error.message}`);
            }
        }

        this.logger.success(`exported ${quipThread.thread.title} to ${path}`)
    }

    /**
     * Get clean title for filename by stripping title prefix if present
     * This prevents duplicate prefixes when re-running exports
     */
    _getCleanTitleForFilename(title) {
        if (!this.options.titlePrefix || !title) {
            return title;
        }

        const prefix = this.options.titlePrefix.trim();
        const trimmedTitle = title.trim();

        // Check if title starts with the prefix (with or without space after)
    ...
    async _updateTitlePrefix(quipThread) {

        if (quipThread.thread.title.startsWith(this.options.titlePrefix)) {
            this.logger.debug(`title for ${quipThread.thread.id} -  - ${quipThread.thread.title} already contains prefix`);
            return;
        }

        try {
            const firstH1 = quipThread.html.match(/<h1[^>]*id\s*=\s*["']([^"']*)["'][^>]*>/i);
            const sectionId = firstH1 ? firstH1[1] : null;

            if (sectionId) {
                const newTitle = this.options.titlePrefix + ' ' + quipThread.thread.title;
                await this.quipService.updateThreadSection(quipThread.thread.id, sectionId, newTitle);
                this.logger.success(`updated title for ${quipThread.thread.id} - ${quipThread.thread.title}`);
            } else {
                this.logger.warn(`no H1 section found in ${quipThread.thread.id} - ${quipThread.thread.title}`);
            }
        } catch (error) {
            this.logger.error(`failed to update title for ${quipThread.thread.id} - ${quipThread.thread.title}: ${error.message}`);
        }
    }
        ...
        for (const index in quipFolder.children) {
            ...
            if (quipChild.thread_id) { //thread
                ...
                if (quipChild.restricted) {
                    this.logger.warn("Folder: " + pathForChildren + " has restricted child: " + quipChild.folder_id);
                } else {
                    folderIds.push(quipChild.folder_id);
                }
            }
